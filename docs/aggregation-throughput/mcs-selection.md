...
为避免外部公式图片加载失败，本文档不再使用 CodeCogs 图片，公式统一使用 GitHub 原生 Markdown 数学公式。注意：GitHub 数学渲染不支持部分 LaTeX 宏，例如 `\operatorname`，因此本文档使用 `\mathrm{erfc}` 等更兼容的写法。
...
其中 `erfc` 是互补误差函数。这里使用 `\mathrm{erfc}` 而不是 `\operatorname{erfc}`，是为了兼容 GitHub 的公式渲染。
...
$$
L_{sub}=14+1500+2=1516\ \mathrm{B}
$$
...
$$
L_{payload}^{agg}\approx10500N_{MPDU}\ \mathrm{B}
$$
...
$$
L_{AMSDU}\approx7\times1516=10612\ \mathrm{B}
$$

每个 MPDU 还要加 MAC header 与 FCS，近似取：

$$
L_{MAC+FCS}\approx30\ \mathrm{B}
$$

则单个 MPDU 长度约为：

$$
L_{MPDU}\approx30+10612=10642\ \mathrm{B}
$$

A-MPDU 中每个 MPDU 前还有 delimiter，近似取：

$$
L_{delimiter}=4\ \mathrm{B}
$$
...
$$
L_{AMPDU,sub}\approx4+10642+2=10648\ \mathrm{B}
$$

如果一次 A-MPDU 聚合 `N_MPDU` 个 MPDU，则：

$$
L_{PSDU}^{agg}\approx10648N_{MPDU}\ \mathrm{B}
$$
